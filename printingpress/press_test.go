...
func TestPrintingPress_WriteHTMLSite(t *testing.T) {
	...
	assert.FileExists(t, outputDir+"/data/nav.js")
	assert.NoFileExists(t, outputDir+"/data/nav.json")
	...
	assert.Contains(t, string(indexHTML), `data-pp-shared="data/nav"`)
	...
	assert.Contains(t, string(indexHTML), `<pb33f-footer fluid url="https://github.com/pb33f/printing-press" link-title="documentation generated by the printing press"></pb33f-footer>`)
	mainStart := strings.Index(string(indexHTML), `<main slot="content">`)
	footerStart := strings.Index(string(indexHTML), `<pb33f-footer`)
	mainEnd := strings.Index(string(indexHTML), `</main>`)
	require.NotEqual(t, -1, mainStart)
	require.NotEqual(t, -1, footerStart)
	require.NotEqual(t, -1, mainEnd)
	assert.Greater(t, footerStart, mainStart)
	assert.Less(t, footerStart, mainEnd)
	...
	assert.Contains(t, string(operationHTML), `data-pp-shared="data/nav"`)
	assert.Contains(t, string(operationHTML), `data-pp-page="data/pages/operations/`+opSlug+`"`)
	assert.Contains(t, string(operationHTML), `const sharedBase = "data/nav";`)
	assert.Contains(t, string(operationHTML), `const pageBase = "data/pages/operations/`+opSlug+`";`)
	assert.Contains(t, string(operationHTML), `<pb33f-footer fluid url="https://github.com/pb33f/printing-press" link-title="documentation generated by the printing press"></pb33f-footer>`)

	sharedJSON, err := os.ReadFile(filepath.Join(outputDir, "data", "nav.js"))
	require.NoError(t, err)
	...
	assert.FileExists(t, filepath.Join(outputDir, "data", "pages", "operations", op.Slug+".js"))
	assert.NoFileExists(t, filepath.Join(outputDir, "data", "pages", "operations", op.Slug+".json"))
	opHTML, err := os.ReadFile(filepath.Join(outputDir, "operations", op.Slug+".html"))
	require.NoError(t, err)
	assert.Contains(t, string(opHTML), `data-pp-page="data/pages/operations/`+op.Slug+`"`)
	...
	assert.FileExists(t, filepath.Join(outputDir, "data", "pages", "models", firstModel.TypeSlug, firstModel.Slug+".js"))
	assert.NoFileExists(t, filepath.Join(outputDir, "data", "pages", "models", firstModel.TypeSlug, firstModel.Slug+".json"))
	modelAssetJSON, err := os.ReadFile(filepath.Join(outputDir, "data", "pages", "models", firstModel.TypeSlug, firstModel.Slug+".js"))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Contains(t, string(modelHTML), `data-pp-page="data/pages/models/`+firstModel.TypeSlug+`/`+firstModel.Slug+`"`)
	...
	if visualModel.MermaidDiagram != "" {
		assert.Contains(t, string(visualHTML), `data-pp-viz-diagram="data/viz/models/`+visualModel.TypeSlug+`/`+visualModel.Slug+`-diagram"`)
		assert.FileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-diagram.js"))
		assert.NoFileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-diagram.json"))
		diagramAssetJS, err := os.ReadFile(filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-diagram.js"))
		require.NoError(t, err)
	...
	if visualModel.GraphJSON != "" {
		assert.Contains(t, string(visualHTML), `data-pp-viz-graph="data/viz/models/`+visualModel.TypeSlug+`/`+visualModel.Slug+`-graph"`)
		assert.FileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-graph.js"))
		assert.NoFileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-graph.json"))
		graphAssetJS, err := os.ReadFile(filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-graph.js"))
		require.NoError(t, err)
...
func TestPrintingPress_WriteHTMLSite_ConfiguresFooter(t *testing.T) {
	...
	pp := newPressEngine(&pressEngineConfig{
		...
		Footer: &FooterConfig{
			...
			LinkTitle: "Docs made by the press",
			Build:     "Generated by API Docs",
		},
	})
	site, err := pp.pressSite()
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Contains(t, string(indexHTML), `<pb33f-footer fluid url="https://example.com/docs" link-title="Docs made by the press" build="Generated by API Docs"></pb33f-footer>`)
}

func TestPrintingPress_WriteHTMLSite_DisablesFooter(t *testing.T) {
	specBytes, err := os.ReadFile("../test_specs/burgershop.openapi.yaml")
	require.NoError(t, err)
	...
	pp := newPressEngine(&pressEngineConfig{
		DrDoc:  drDoc,
		Title:  "Burger Shop",
		Footer: &FooterConfig{Disabled: true},
	})
	site, err := pp.pressSite()
	require.NoError(t, err)
	...
	indexHTML, err := os.ReadFile(filepath.Join(outputDir, "index.html"))
	require.NoError(t, err)
	assert.NotContains(t, string(indexHTML), `<pb33f-footer`)
}
...
func TestPrintingPress_WriteHTMLSite_UsesConfigOutputDirAndBaseURL(t *testing.T) {
	...
	assert.Contains(t, string(indexHTML), `data-pp-shared="/docs/data/nav"`)
	assert.Contains(t, string(indexHTML), `const sharedBase = "/docs/data/nav";`)
	...
	assert.Contains(t, string(operationHTML), `data-pp-shared="/docs/data/nav"`)
	assert.Contains(t, string(operationHTML), `data-pp-page="/docs/data/pages/operations/`+opSlug+`"`)
	assert.Contains(t, string(operationHTML), `const sharedBase = "/docs/data/nav";`)
	assert.Contains(t, string(operationHTML), `const pageBase = "/docs/data/pages/operations/`+opSlug+`";`)
...
func TestPrintingPress_WriteHTMLSite_CleansStaleHydrationAssetsBetweenModes(t *testing.T) {
	...
	assert.FileExists(t, filepath.Join(outputDir, "data", "pages", "operations", opSlug+".js"))

	legacyAssets := []string{
		filepath.Join(outputDir, "static", "printing-press-shared.js"),
		filepath.Join(outputDir, "static", "printing-press-shared.json"),
		filepath.Join(outputDir, "static", "page-data", "operations", opSlug+".js"),
		filepath.Join(outputDir, "static", "page-viz", "models", "schemas", "legacy-diagram.js"),
	}
	for _, legacyAsset := range legacyAssets {
		require.NoError(t, os.MkdirAll(filepath.Dir(legacyAsset), 0o755))
		require.NoError(t, os.WriteFile(legacyAsset, []byte("stale"), 0o644))
		require.NoError(t, os.WriteFile(legacyAsset+".gz", []byte("stale gzip"), 0o644))
	}
	...
	assert.FileExists(t, filepath.Join(outputDir, "data", "nav.json"))
	assert.NoFileExists(t, filepath.Join(outputDir, "data", "nav.js"))
	assert.FileExists(t, filepath.Join(outputDir, "data", "pages", "operations", opSlug+".json"))
	assert.NoFileExists(t, filepath.Join(outputDir, "data", "pages", "operations", opSlug+".js"))
	for _, legacyAsset := range legacyAssets {
		assert.NoFileExists(t, legacyAsset)
		assert.NoFileExists(t, legacyAsset+".gz")
	}
	...
	require.NotNil(t, visualModel)
	if visualModel.MermaidDiagram != "" {
		assert.FileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-diagram.json"))
		assert.NoFileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-diagram.js"))
	}
	if visualModel.GraphJSON != "" {
		assert.FileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-graph.json"))
		assert.NoFileExists(t, filepath.Join(outputDir, "data", "viz", "models", visualModel.TypeSlug, visualModel.Slug+"-graph.js"))
	}
}
